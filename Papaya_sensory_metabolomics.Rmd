...
```{r Sugar_transformation,  cache.lazy=FALSE, dependson="Sugar extraction efficiency", collapse=TRUE}
...
```{r GTR data, warning=FALSE,  cache.lazy=FALSE, dependson="Sugar_transformation", collapse=TRUE}
...
```{r combine data,  cache.lazy=FALSE, collapse=TRUE, dependson=c("GTR data","Sugar_transformation","OC transformations")}
...
# missing results
kable(conc_matrix[!complete.cases(conc_matrix),],
...
```{r Sensory modelling,  results='hide', message=FALSE, warning=FALSE}
...
save(wts, "data/sensory_modelling.RData")
...
kable(blue_df, caption = "Estimated marginal means from the mixed effects model including genotype as a fixed effect and participant as a random effect", 
...
```{r liking variable influence,  results='hide', warning=FALSE}
...
save(results, "data/liking_var_model.RData")
...
sens_table %>%
    gt() %>%
    tab_caption("Sensory decomposition")
```
...
mets_table%>%
    gt() %>%
    tab_caption("Metabolite decomposition")
...
save(dtrain, dtest, results, predictions, xgb_model, "data/xgboost_train_results.RData")
```
