...
The aim of this project was to determine the role of flavour related metabolites in papaya fruit flavour, focussing on taste and aroma. As demonstrated by @Colantonio2022, metabolomic analysis of fruit can identify metabolite markers for fruit flavour selection in breeding programs. Subsequently, genetic markers may be developed, which are neccesary for papaya breeding due to the challenges associated with growing thousands of mature trees for phenotyping [@Lomax2024]. We aimed to build on the work reported by @Zhou2022
...
Target VOCs were selected based on the concentration ranges reported @Brewer2021, focusing on species that exceeded their odour threshold values in water (OAV > 1). The SPME optimisation and GC-MS conditions largely emulated @Pico2022.
...
```{r meta data, cache=F, collapse=TRUE}
...
```{r VOC Matrix Effect, collapse=TRUE, cache=TRUE}
...
```{r VOC transformations, collapse=TRUE, cache=TRUE, dependson="VOC Matrix Effect"}
...
```{r Sugar extraction efficiency, collapse=TRUE, cache=TRUE}
...
```{r Sugar_transformation, cache=TRUE, cache.lazy=FALSE, dependson="Sugar extraction efficiency", collapse=TRUE}
...
```{r GTR data, warning=FALSE, cache=TRUE, cache.lazy=FALSE, dependson="Sugar_transformation", collapse=TRUE}
...
```{r combine data, cache=TRUE, cache.lazy=FALSE, collapse=TRUE, dependson=c("GTR data","Sugar_transformation","OC transformations")}
...
```{r PCA sensory, fig.width=10, fig.height=8, fig.dpi=600, cache=TRUE}
# data set up
...
```{r PCA metabolites, fig.width=10, fig.height=8, fig.dpi=600, cache=TRUE}
PCA_vocs <- PCA(PCA_data, scale.unit = TRUE, 
                ncp = 6, graph = F, 
...
```{r PCA metabolites new, fig.width=10, fig.height=8, fig.dpi=600, cache=TRUE}
PCA_vocs <- PCA(PCA_data, scale.unit = TRUE,
                ncp = 6, graph = F,
...
```{r Sensory modelling, cache=TRUE, results='hide', message=FALSE, warning=FALSE}
...
```{r aroma metabolites, cache = TRUE, fig.height= 10, fig.width = 8, fig.dpi = 600}
# Plot prep ------------
bglr_data <- wts %>% 
...
```{r Flavour metabolites, cache = TRUE, fig.height= 10, fig.width = 8, fig.dpi = 600}
...
```{r aftertaste metabolites, cache = TRUE, fig.height= 10, fig.width = 8, fig.dpi = 600}
...
```{r texture metabolites, cache = TRUE, fig.height= 10, fig.width = 8, fig.dpi = 600}
...
```{r Consumer cluster analysis, results = FALSE, cache=TRUE, fig.height=7, fig.width=10, fig.dpi=600}
...
```{r Consumer cluster PCA, cache=TRUE, fig.height=6, fig.width=8, fig.dpi=600}
...
```{r Consumer mixed model, cache=TRUE}
...
```{r RS5 comparison, cache=TRUE, fig.height=6, fig.width=8, dpi=600}
...
```{r liking variable influence, cache=TRUE, results='hide', warning=FALSE}
...
```{r plot liking variables, cache=TRUE}
...
```{r metabolite liking plot, cache=TRUE, fig.height=6, fig.width=5, dpi=600}
metabolites_plot
```
...
```{r sensory liking plot, cache=TRUE, fig.height=6, fig.width=5, dpi=600}
Sensory_plot
```
...
```{r variance decomposition, cache=TRUE}
# update model data
...
Finally a predictive model was generated to predict papaya fruit liking in future selection processes. The variables included in the model were reduced to four to prevent overfitting due to the limited sample size: sulcatone (terpene), β-ionone (sesquiterpene), γ-Octalactone (lipid-derived) and Brix (total soluble solids; linked to total sugars). These variables were selected from variable influence plots in the previous section. This model may be implemented for selection purposes if the sampling and instrumentation protocols are repeated. New samples may be indexed within the scores of samples used to build the model. Considering the model isn't equipped for extrapolation, any values outside the ranges of what was used to build the model may distort the predicted liking score. XGBoost has inherent randomness that isn't controlled by set.seed(), therefore reproducibility may not be perfect.

```{r xgboost parameter validation, eval=FALSE}
fit_papaya_liking_model <- function(boost_data,
  ...
  # Data preparation
  model_data <- boost_data |> scale() |> as.data.frame()
  
  # Train-test split
  set.seed(seed)
  train_index <- createDataPartition(model_data$emmean, p = 1 - test_prop, list = FALSE)[, 1]
  
  X_train_full <- as.matrix(model_data[train_index, -1])
  X_test <- as.matrix(model_data[-train_index, -1])
  y_train_full <- model_data$emmean[train_index]
  y_test <- model_data$emmean[-train_index]
...
perform_cv_validation <- function(X_train_full, y_train_full, params, nrounds, cv_folds, seed) {
  ...
  for (fold in 1:cv_folds) {
    ...
    fold_metrics <- calculate_fold_metrics(
      ...
      best_nrounds = model_cv$best_iteration
    )
...
boost_data <- model_data |> select(emmean,Brix,Gamma_octalactone,Beta_ionone,Sulcatone)
# start modelling
results <- fit_papaya_liking_model(boost_data, seed = 11, cv_folds = 5)

saveRDS(results,"Output/xgboost_validation.RDS")
```

```{r XGBoost test model}
results <- readRDS("Output/xgboost_validation.RDS")
...
final_model_results <- results$train_final_model(selected_row)
...
```{r XGboost eval, cache=TRUE}
# Model evaluation
final_model_results$metrics %>%
  pivot_longer(
    cols = everything(),
    names_to = "metric",
    values_to = "value"
  ...
  select(dataset, n, mae, rmse, r2, adj_r2, nrmse, corr, rpd, mean_residual, residual_std) |> 
  mutate(across(where(is.numeric),~round(.x,2))) |> 
  gt()
```

```{r XGBoost pred vs obs, cache=TRUE, fig.height=7, fig.width=7}
...
### Figure 13: XGBoost model predicted liking vs observed liking based on concentrations of sulcatone, Gamma octalactone, Beta ionone, and Brix.

```{r feature importance plot, cache=TRUE}
# Feature Importance
xgb.plot.importance(final_model_results$importance,
...
```{r feature importance table, cache=TRUE}
xgb.plot.importance(final_model_results$importance,
                    main = "Feature Importance Plot") |> 
    gt() %>%
    tab_header(
...
```{r data summary by genotype, echo=FALSE}
data_summary_genotype %>% 
  datatable()
```
