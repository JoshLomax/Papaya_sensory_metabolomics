...
boost_data <- model_data |> 
  select(emmean,Brix,Gamma_octalactone,Beta_ionone,Sulcatone) |> 
  scale() |> 
  as.data.frame()

# start modelling - MAIN COMMAND
# results <- fit_papaya_liking_model(boost_data, seed = 11, cv_folds = 5)

# saveRDS(results,"data/xgboost_validation.RDS")
```

```{r XGBoost-test-model}
results <- readRDS("data/xgboost_validation.RDS")
...
gt(performance_table) |>
  tab_caption("XGBoost model performance metrics on Z-scaled data")
...
ggplot(data.frame(Predicted = predictions, 
                        Actual = boost_data[results$data_splits$test_idx,"emmean"]), 
