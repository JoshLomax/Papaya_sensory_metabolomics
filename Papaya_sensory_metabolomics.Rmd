...
FL_bitterness <- c("γ-Octalactone", "Sucrose","Hexanal","(E/Z)-Geranylacetone","β-Ionone","Glucose","Terpinolene","Water fraction","Brix")
...
writexl::write_xlsx(model_results,"results/model_results.xlsx")
...
```{r models_table}
# Comparison table
read_xlsx("data/model_results.xlsx") %>%
...
```{r xgboost fine-tuning, eval=FALSE}
# Function to run 5-fold cross-validation using hyperparameter grid search
run_xgb_cv <- function(params, dtrain, nrounds = 500, nfold = 5, early_stopping = 50) {
  cv <- xgb.cv(
    params = params,
...
boost_data <- model_data %>%
    select(emmean, Sucrose, wet_fraction, Linalool_oxide, Gamma_octalactone)
...
```{r XGBoost test model, eval=FALSE}
...
### Figure 13: XGBoost model predicted liking vs observed liking based on concentrations of sulcatone, γ-Octalactone, linalool oxide and total sugar.

```{r feature importance plot, eval=FALSE}
# Feature Importance
...
```{r feature importance table, eval=FALSE}
