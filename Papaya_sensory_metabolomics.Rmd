...
required_packages <- c(
    ...
    "ggpmisc","ggthemes","ggrepel","geomtextpath","scales", "ggcorrplot", # Plotting stuff

    "FactoMineR","factoextra", "ggcorrplot", # PCA & visualization

    "car","agricolae","tableone", "summarytools", "rstatix", # Stats & summaries
...
sensory_meta <- 
  ...
  mutate(
    group = snakecase::to_title_case(group),
    label = var %>%
      str_remove("_[A-Z]+$") %>%
      str_replace_all("_", " ") %>%
      tolower(),
...
```{r VOC-transformations, collapse=TRUE}
...
```{r Sugar-transformation, collapse=TRUE}
...
# saveRDS(imp, "data/sugar_imp.RDS")
#imp <- readRDS("data/sugar_imp.RDS")
...
```{r GTR-data, warning=FALSE, collapse=TRUE}
...
# missing results
kbl(conc_matrix[!complete.cases(conc_matrix), ], caption = "Results with missing values", digits = 1) %>%
  kable_styling()
...
```
## Explanatory variable colinearity

```{r sensory-collinearity, fig.cap="Correlation plot showing relationships between sensory variables. Purple indicates positive correlations, green indicates negative correlations, and white indicates no correlation. Only significant correlations (p < 0.05) are shown.", fig.height=8, fig.width=8}

# Data preparation
sens_vars <- full_data_matrix |>
  select(ID, aroma_intensity_AR:prickly_AT) |>
  pivot_longer(aroma_intensity_AR:prickly_AT, names_to = "var", values_to = "score") |>
  left_join(sensory_meta) |>
  select(ID, label2, score) |>
  pivot_wider(names_from = "label2", values_from = "score") |> 
  select(-ID)
...
ggcorrplot(
  ...
  labs(
    title = "Correlations Between Sensory Attribute Scores",
    caption = "* Blank spaces indicate non-significant correlations (p > 0.05)"
  )

```

```{r metabolites-collinearity, fig.cap="Correlation plot showing relationships between metabolite variables. Purple indicates positive correlations, green indicates negative correlations, and white indicates no correlation. Only significant correlations (p < 0.05) are shown.", fig.height=8, fig.width=8}

# Data preparation
mets_vars <- full_data_matrix |>
...
ggcorrplot(
  ...
  labs(
    title = "Correlations Between Metabolite Concentrations",
    caption = "* Blank spaces indicate non-significant correlations (p > 0.05)"
  )


# ggsave("mets_corr.png",
#        width = 8,
#        height = 7,
#        dpi = 600,
#        bg = "white",
...
Finally a predictive model was generated to predict papaya fruit liking in future selection processes. Due to the limited sample size, feature selection based on Figure 14 statistics was implemented. variables are ranked by GBM variable importance and top ranking variables are included. The number of variables are compared to optimise model performance and reduce over-fitting (estimated by test-train metrics concordance).
...
```{r compare-variable-subsets, eval=FALSE}
important_variables <- results |> filter(model == "Red_Yellow_mets_gbm") |> arrange(desc(coord))

variable_combos <- c(2,3,4,5,6,7,8)

combo_results <- variable_combos |>
  set_names() |>
  map(function(top_variables) {
    explanatory_vars <- important_variables |>
      slice_head(n = top_variables) |>
      pull(var)
    
    boost_data <- model_data |>
      select(emmean, all_of(explanatory_vars)) |>
      scale() |>
      as.data.frame()
    ...
    final_model_results <- xgb_results$train_final_model(selected_row)
...
saveRDS(combo_results, "data/xgb_combos.RDS")
```

```{r xgb-combo-results}
combo_results <- readRDS("data/xgb_combos.RDS")

combo_results |> 
  gt() |> 
  tab_caption("XGBoost model performance metrics on Z-scaled data for 2 - 8 explanatory variables")

```

```{r XGBoost-final-model, results='hide'}

boost_data <- model_data |>
  select(emmean, Sulcatone, Brix, Beta_ionone) |>
  scale() |>
  as.data.frame()
...
ggplot(data.frame(Predicted = predictions, 
                        Actual = boost_data[final_xgb_results$data_splits$test_idx,"emmean"]), 
...
final_model_results$importance %>%  
    gt() %>%
...
left_join(read_xlsx("data/VOC_limits.xlsx", sheet = "Sheet2"), VOC_summary) %>% 
  mutate(across(where(is.numeric), ~ round(.x, digits = 2))) %>%
  kbl(
    caption = "Summary of VOC concentrations (ppb) by chemical species",
    col.names = c("Species", "Chemical Class", "CAS Number", "MRM transition", "RT", "RI", "LOD", "LOQ", "n", "Mean", "SD", "SE", "Median", "Min", "Max", "N detected", "% detected"),
